...
COPY INTO precipitation_tmp FROM @my_json_stage/USW00023169-LAS_VEGAS_MCCARRAN_INTL_AP-precipitation-inch.csv file_format=(type=csv field_delimiter=',' skip_header=1);

COPY INTO temperature_tmp FROM @my_json_stage/USW00023169-temperature-degreeF.csv file_format=(type=csv field_delimiter=',' skip_header=1);
...
SELECT TO_DATE(date_tmp,'MM/DD/YYYY'), 
CAST(precipitation_tmp AS FLOAT), 
CAST(precipitation_normal AS FLOAT) FROM "UDACITYPROJECT"."STAGING".precipitation_tmp;
...
CAST(normal_max_tmp AS FLOAT) FROM "UDACITYPROJECT"."STAGING".temperature_tmp;
...
SELECT parse_json($1):business_id, 
	...
	parse_json($1):text, 
	parse_json($1):useful, 
	parse_json($1):user_id FROM "UDACITYPROJECT"."STAGING".review_tmp
...
SELECT 
	...
	parse_json($1):"highlights" FROM "UDACITYPROJECT"."STAGING".covid_tmp;
...
CREATE TABLE covid CLONE "UDACITYPROJECT"."ODS".covid ;

CREATE TABLE tip CLONE "UDACITYPROJECT"."ODS".tip;

CREATE TABLE user CLONE "UDACITYPROJECT"."ODS".user ;

CREATE TABLE review CLONE "UDACITYPROJECT"."ODS".review;
...
CREATE TABLE total_info CLONE "UDACITYPROJECT"."ODS".total_info;
